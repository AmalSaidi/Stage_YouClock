...
@once
      <button type="submit" class="btn btn-danger" id="vuebutton" style="margin-bottom: 1%;"><a style="color:white;"href="/FicheHoraire/Details/{{$employe->id}}/{{$aff->idfiche}}">Retour</a></button>
@endonce
@endforeach
...
<div id="calendar">
  <table class="table-bordered" id="MyTable" style="margin-top: 1%;">
  <thead>
    <th>Date</th>
    <th>Activité</th>
    <th>Matin</th>
  ...
  @foreach( $affichage as $aff)
    @if($employe->nom==$userNom)
      ...
      <td><input name="date" type="hidden" value="{{ $aff->Date }}"> {{ $aff->Date }}</input></td>
        ...
        <td><a href = '/edit/utilisateur/{{ $aff->id }}/{{ $aff->idUser }}'><button id="pointer">Pointer</button></a></td>
        @else
        <td><a href = '/edit/utilisateur/{{ $aff->id }}/{{ $aff->idUser }}'><button id="pointer" disabled>Pointer</button></a></td>
        @endif
      </tr>
